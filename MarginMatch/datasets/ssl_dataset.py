...
mean['mini-imagenet'] = [0.485, 0.456, 0.406]
mean['tiered-imagenet'] = [0.485, 0.456, 0.406]
mean['CUB_200_2011'] = [0.485, 0.456, 0.406]
...
std['mini-imagenet'] = [0.229, 0.224, 0.225]
std['tiered-imagenet'] = [0.229, 0.224, 0.225]
std['CUB_200_2011'] = [0.229, 0.224, 0.225]
import pickle as pkl
from others.EP.datasets.utils import load_semi_data_validation
import cv2
...
class SSL_Dataset:
    ...
    def __init__(self,
        ...
        if self.name.lower() == 'mini-imagenet' or self.name.lower() == 'tiered-imagenet' or self.name.lower() == 'cub_200_2011':
            crop_size = 84
        self.transform = get_transform(mean[name], std[name], crop_size, train)
        self.split_train = 'train-test-seed-10-K-'
    ...
    def get_data(self, svhn_extra=True):
        ...
        """
        if self.name.lower() == 'mini-imagenet' or self.name.lower() == 'tiered-imagenet' or self.name.lower() == 'cub_200_2011':
            split_semi = '{}{:03d}'.format(self.split_train, self.args.num_per_class)
            split = 'train'
            if self.name.lower() == 'mini-imagenet':
                data_root = os.path.join(self.data_dir, self.name, "mini-imagenet-cache-%s.pkl")
                with open(data_root % 'train', 'rb') as infile:
            ...
            elif self.name.lower() == 'tiered-imagenet':
                data_root = os.path.join(self.data_dir, self.name, "%s_labels.pkl")
                split = 'train'
                img_path = os.path.join(self.data_dir, self.name, "%s_images_png.pkl" % (split))
                label_path = os.path.join(self.data_dir, self.name, "%s_labels.pkl" % (split))
            ...
            elif self.name.lower() == 'cub_200_2011':
                data_root = os.path.join(self.data_dir, self.name, "cub-%s.pkl")
                split_semi = split_semi.replace(split, 'base')
                split = 'base'
                with open(data_root % split, 'rb') as infile:
                    data = pkl.load(infile)
                labels = np.array(data['labels'])
                features = np.asarray(data['features'])
                data_root = os.path.join(self.data_dir, self.name, "%s.pkl")
                label_map = {l: i for i, l in enumerate(sorted(np.unique(labels)))}
                labels = np.array([label_map[l] for l in labels])

            if self.train:
                data_group = 'train_label'
                data, targets = load_semi_data_validation(data_root, features, labels, split_semi,
                                                                 data_group)
                data_group = 'train_unlabel'
                ulb_data, _ = load_semi_data_validation(data_root, features, labels, split_semi,
                                                          data_group)
            else:
    ...
    def get_dset(self, is_ulb=False,
        ...
        if self.name.upper() == 'STL10' or self.name.lower() == 'mini-imagenet' or \
                self.name.lower() == 'tiered-imagenet' or self.name.lower() == 'cub_200_2011':
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        if self.name.upper() == 'STL10':
            ...
            ulb_targets = None
        elif self.name.lower() == 'mini-imagenet' or self.name.lower() == 'tiered-imagenet' or self.name.lower() == 'cub_200_2011':
            lb_data, lb_targets, ulb_data = self.get_data()
            ulb_targets = None
        else:
            data, targets = self.get_data()
