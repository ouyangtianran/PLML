...
from models.marginmatch.marginmatch import MarginMatch
...
def main(args):
    ...
    #                          '{}_{}_{}_npc_{:03d}'.format(args.save_name, args.dataset, args.net, args.num_per_class))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    save_path = os.path.join(args.save_dir, args.save_name)
    # save_path = os.path.join(args.save_dir, '{}_{}_{}_npc_{:03d}'.format(args.save_name, args.dataset, args.net, args.num_per_class))
    ...
    if args.use_threshold_examples is True:
      net_classes = args.num_classes + 1
    else:
      net_classes = args.num_classes

    args.bn_momentum = 1.0 - 0.999
    if args.net == 'conv4':
        _net_builder = Conv4
    elif args.net == 'resnet12':
        _net_builder = Resnet12
    ...
    model = MarginMatch(_net_builder,
                     net_classes,
                     args.ema_m,
                     args.T,
                     args.p_cutoff,
    ...
    ## set SGD and cosine lr on MarginMatch
    ...
    train_dset = SSL_Dataset(args, alg='marginmatch', name=args.dataset, train=True,
                            num_classes=args.num_classes, data_dir=args.data_dir)
    lb_dset, ulb_dset = train_dset.get_ssl_dset(args.num_labels)
    _eval_dset = SSL_Dataset(args, alg='marginmatch', name=args.dataset, train=False,
                            num_classes=args.num_classes, data_dir=args.data_dir)
    ...
    model.ulb_dset = ulb_dset
    model.set_data_loader(loader_dict)
    model.set_dset(train_dset, eval_dset)

    # If args.resume, load checkpoints from args.load_path
    if args.resume:
        model.load_model(args.load_path)

    # START TRAINING of marginmatch
    trainer = model.train
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sn', '--save_name', type=str, default='MarginMatch')
    ...
    '''
    Training Configuration of marginmatch
    '''
    ...
    parser.add_argument('--data_dir', type=str, default='./EP_data')
    ...
    AUM Config
    '''
    parser.add_argument('--aum_threshold', default=-1.25, type=float,
                        help='AUM threshold')
    parser.add_argument('--delta', default=0.997, type=float,
                        help='EMA smoothing for marigin calculation')
    parser.add_argument('--switcher_frequency', default=10000, type=int,
                        help='Dataset switch frequency')
