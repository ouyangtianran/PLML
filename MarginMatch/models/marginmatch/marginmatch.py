...
class MarginMatch:
    ...
    def train(self, args, logger=None):
        ...
        selected_label_confidence = torch.ones((len(self.ulb_dset),), dtype=torch.long, ) * -1
        selected_label_confidence = selected_label_confidence.cuda(args.gpu)
        
        classwise_acc_confidence = torch.zeros((self.num_classes,)).cuda(args.gpu)

        aum_calculator = AUMCalculator(args.delta, int(self.num_classes), len(self.ulb_dset), args.percentile / 100)
