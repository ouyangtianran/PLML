...
class EmbeddingPropagation(torch.nn.Module):
    ...
    def forward(self, x, propagator=None, knn=0):
        return embedding_propagation(x, self.alpha, self.rbf_scale, self.norm_prop, propagator=propagator, knn=knn)
...
class LabelPropagation(torch.nn.Module):
    ...
    def forward(self, x, labels, nclasses, propagator=None, labels_weight=None, knn=0):
        ...
        return label_propagation(x, labels, nclasses, self.alpha, self.rbf_scale,
                                 ...
                                 balanced=self.balanced, labels_weight=labels_weight, knn=knn)


def get_similarity_matrix(x, rbf_scale, k=0):
    b, c = x.size()
    ...
    if k > 0:
        t = torch.topk(weights, k=k, dim=1)
        ind = t.indices
        mask = torch.zeros([b,b], dtype=torch.bool, device=weights.device)
        for i in range(b):
            mask[i][ind[i]] = True
...
def embedding_propagation(x, alpha, rbf_scale, norm_prop, propagator=None, knn=0):
    if propagator is None:
        weights = get_similarity_matrix(x, rbf_scale, knn)
        propagator = global_consistency(
            weights, alpha=alpha, norm_prop=norm_prop)
    return torch.mm(propagator, x)


def label_propagation(x, labels, nclasses, alpha, rbf_scale, norm_prop, apply_log, propagator=None, balanced=False, epsilon=1e-6, labels_weight=None, knn=0):
    labels = F.one_hot(labels, nclasses + 1)
    labels = labels[:, :nclasses].float()  # the max label is unlabeled
    if labels_weight is not None:
        labels = torch.mm(torch.diag_embed(labels_weight), labels)
    if balanced:
...
def global_consistency(weights, alpha=1, norm_prop=False):
    ...
    propagator = torch.inverse(propagator[None, ...].to('cpu')).to('cuda')[0]
