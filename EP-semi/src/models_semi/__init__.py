...
import pickle, os
import numpy as np
from . import pretraining, finetuning, ssl_wrapper, semi_pretraining, semi_finetuning, semi_ssl_wrapper, noise_finetuning

def get_model(model_name, backbone, n_classes, exp_dict, pretrained_embs=None, pretrained_weights_dir=None, savedir_base=None):
    if model_name == "pretraining":
        model = pretraining.PretrainWrapper(backbone, n_classes, exp_dict, pretrained_embs)

    elif model_name == "semi_pretraining":
        model =semi_pretraining.PretrainWrapper(backbone, n_classes, exp_dict, pretrained_embs)

    elif model_name == "finetuning":
        model = finetuning.FinetuneWrapper(backbone, n_classes, exp_dict, pretrained_embs)

    elif model_name == "semi_finetuning":
        model = semi_finetuning.FinetuneWrapper(backbone, n_classes, exp_dict, pretrained_embs)

    elif model_name == "noise_finetuning":
        model = noise_finetuning.FinetuneWrapper(backbone, n_classes, exp_dict, pretrained_embs)
        
    elif model_name == "ssl":
        model = ssl_wrapper.SSLWrapper(backbone, n_classes, exp_dict, savedir_base=savedir_base)
    elif model_name == "semi_ssl":
        model = semi_ssl_wrapper.SSLWrapper(backbone, n_classes, exp_dict, savedir_base=savedir_base)
