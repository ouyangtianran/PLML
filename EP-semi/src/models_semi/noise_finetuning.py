...
from embedding_propagation import EmbeddingPropagation, LabelPropagation, get_similarity_matrix
...
from .utils import get_proto, get_pseudo_label, top_accuracy, remove_prefix, remove_extra_dim,remove_extra_dim_1
...
from src.models.base_ssl.selection_methods.kmeans import MultiFactorClustering_logit
...
class FinetuneWrapper(BaseWrapper):
    ...
    def __init__(self, model, nclasses, exp_dict, pretrained_embs= None, n_word_emb=300):
        ...
        # using negative embedding to process noise samples
        self.model.neg_embedding = torch.nn.Embedding(nclasses, self.model.output_size)
        ...
        if self.exp_dict["pretrained_weights_root"] is not None:
            ...
            if pretrain_models_flag in self.exp_dict["pretrained_weights_root"]:
                ...
                for model_pth in os.listdir(self.exp_dict['pretrained_weights_root']):
                    if pth_name in model_pth:
                        if pm == 'semco':
                            ...
                            # model_static_dict = torch.load(base_path)['model']
                            model_static_dict = torch.load(base_path)['ema_model']
                            model_static_dict = remove_extra_dim(model_static_dict)
                            # model_static_dict = remove_extra_dim_1(model_static_dict)
                        self.model.load_state_dict(model_static_dict, strict=False)
                ...
                for exp_hash in os.listdir(self.exp_dict['pretrained_weights_root']):
                    ...
                    if ((loaded_exp_dict["model"]["name"] == 'pretraining' or loaded_exp_dict["model"]["name"] == 'semi_finetuning'
                    or loaded_exp_dict["model"]["name"] == 'semi_pretraining') and
    ...
    def get_logits(self, embeddings, support_size, query_size, nclasses, labels_weight=None):
        ...
        if self.exp_dict["distance_type"] == "labelprop":
            ...
            if self.model.training:
                knn = self.exp_dict["lp_knn"]
            else:
                knn = 0
            logits = self.label_propagation(embeddings, labels, nclasses, labels_weight=labels_weight, knn=knn)
    ...
    def get_similiarity(self, x1, x2, rbf_scale=0.1):
        if rbf_scale > 0:
            dist = self.pdist(x1, x2)
            sim = torch.exp(-dist * rbf_scale).view(-1)
        elif rbf_scale == 0: # cos similarity
            sim = (self.cos_sim(x1, x2) + 1) /2.0
        else:
            raise ValueError("Distance type not recognized")

        return sim
    def get_query_label_weight(self, embeddings, embeddings_smooth, n_neg_embeddings, support_size, nclasses, rbf_scale=0.1):
        b, c = embeddings.size()
        embeddings = embeddings.view(-1, nclasses, c)
        query_embeddings = embeddings[support_size:].view(-1, c)
        embeddings_s = embeddings_smooth.view(-1, nclasses, c)
        query_embeddings_s = embeddings_s[support_size:].view(-1, c)
        weight = self.get_similiarity(query_embeddings, query_embeddings_s, rbf_scale).view(-1)

        return weight

    def get_emb_label_weight(self, embeddings, embeddings_smooth, rbf_scale=0.1):
        weight = self.get_similiarity(embeddings, embeddings_smooth, rbf_scale).view(-1)

        return weight


    def get_logits_with_noise(self, embeddings, support_size, query_size, nclasses, neg_embeddings, noise_labels):
        """Computes the logits from the queries of an episode
        ...
        if self.exp_dict["distance_type"] == "labelprop":
            support_labels = torch.arange(nclasses, device=embeddings.device).view(1, nclasses).repeat(support_size,
                                                                                                       1).view(
                support_size, nclasses)
            unlabeled_labels = (nclasses +1) * torch.ones(query_size * nclasses, dtype=support_labels.dtype,
                                                     device=support_labels.device).view(query_size, nclasses)
            labels = torch.cat([support_labels, unlabeled_labels], 0).view(-1)
            n_labels = len(labels)
            if b > n_labels:
                unlabeled_labels2 = (nclasses +1) * torch.ones(b - n_labels, dtype=support_labels.dtype,
                                                          device=support_labels.device).view(-1)
                labels = torch.cat([labels, unlabeled_labels2], 0).view(-1)
            # add labels for negative embedding
            # noise_labels = nclasses * torch.ones(nb, dtype=support_labels.dtype,
            #                                      device=support_labels.device).view(-1)
            labels = torch.cat([labels, noise_labels])
            embeddings2 = torch.cat([embeddings, neg_embeddings],0)
            logits = self.label_propagation(embeddings2, labels, nclasses+1)

            logits1 = logits[:n_labels].view(-1, nclasses, nclasses+1)[support_size:(support_size + query_size),
                      ...].view(-1,
                                nclasses+1)
            logits2 = logits[n_labels:]
            logits = torch.cat([logits1, logits2], 0)
    ...
    def resort_support(self, embeddings, support_size, query_size, nclasses, c, labels, labels_gt):
        embeddings = embeddings.view(support_size + query_size, nclasses, -1)
        anchor = embeddings[0].view(1, nclasses, c)
        cand = embeddings[1:]
        with torch.no_grad():
            d = cand - anchor
            d = torch.sum(d ** 2, 2)
            ind = torch.argsort(d,dim=0)
        cand_list = []
        for i in range(nclasses):
            c0 = torch.index_select(cand[:,i,:], dim=0, index=ind[:,i]).unsqueeze(dim=1)
            cand_list.append(c0)
        cand = torch.cat(cand_list, dim=1)
        embeddings = torch.cat([anchor,cand],dim=0)
        if labels_gt is not None:
            labels_gt = labels_gt.view(support_size + query_size, nclasses, -1)
            labels = labels.view(support_size + query_size, nclasses, -1)
            supp_labels = labels[:support_size]
            supp_labels_gt = labels_gt[:support_size]
            v_measure0 = metrics.v_measure_score(supp_labels_gt.view(-1).numpy(),supp_labels.view(-1).cpu().numpy())
            # resort labels_gt
            gt0 = labels_gt[0].view(1,nclasses)
            cand_gt = labels_gt[1:]
            cand_gt_list = []
            ind = ind.cpu()
            for i in range(nclasses):
                gt1 = torch.index_select(cand_gt[:,i], dim=0, index=ind[:,i])
                cand_gt_list.append(gt1)
            cand_gt = torch.cat(cand_gt_list,dim=1)
            resort_gt = torch.cat([gt0, cand_gt])
            supp_labels_gt = resort_gt[:support_size]
            v_measure1 = metrics.v_measure_score(supp_labels_gt.view(-1).numpy(),
                                                 supp_labels.view(-1).cpu().numpy())
        else:
            v_measure0, v_measure1 = 0, 0
        return embeddings.view(-1, c), v_measure0, v_measure1
    def query_drop(self, logits, query_labels, nclasses, p, drop_type='random'):
        logits = logits.view(-1,nclasses,nclasses)
        query_labels = query_labels.view(-1, nclasses)
        n = query_labels.shape[0]
        n_keep = int(n*(1-p))
        if drop_type == 'random':
            ind = torch.randperm(n_keep, device=logits.device)
        else:
            ind = torch.arange(0, n_keep, device=logits.device)
        logits = logits[ind]
        query_labels = query_labels[ind]
    ...
    def train_on_batch(self, batch):
        ...
        v_measure0, v_measure1 = 0, 0
        if self.exp_dict["cluster-FSL"] == False:
            x = torch.cat([tx, vx], 0)
            ...
            labels_gt = episode["targets_gt"]
            if self.exp_dict["resort"]==True:
                embeddings, v_measure0, v_measure1 = self.resort_support(embeddings, support_size, query_size, nclasses, c, labels, labels_gt)
            else:
                v_measure0, v_measure1 = 0, 0
            emb0 = embeddings
            ...
            if self.exp_dict["embedding_prop"] == True:
                embeddings = self.embedding_propagation(embeddings, knn=self.exp_dict["ep_knn"])
            ...
            query_labels = torch.arange(nclasses, device=embeddings.device).view(1, nclasses).repeat(query_size, 1).view(-1)
            if self.exp_dict["query_drop_rate"] > 0:
                logits, query_labels = self.query_drop(logits, query_labels, nclasses, self.exp_dict["query_drop_rate"],
                                                       self.exp_dict["drop_type"])  #
            loss += F.cross_entropy(logits, query_labels) * self.exp_dict["few_shot_weight"]
            # if self.exp_dict["logits_consistency_weight"]>0:
            #     emb_q = embeddings.view(-1, nclasses, c)[support_size:(support_size + query_size),
            #               ...].view(-1,c)
            #     weights = get_similarity_matrix(emb_q, rbf_scale=1, k=self.exp_dict["logits_consistency_knn"])
            #     logits_w = weights @ logits
            #     loss += F.kl_div(F.log_softmax(logits), F.softmax(logits_w)) * self.exp_dict["logits_consistency_weight"]
        else:
            augmented_tx = episode["augmented_set"][:support_size, ...].view(support_size, nclasses, c, h, w).cuda(
                non_blocking=True)
            x = torch.cat([augmented_tx, vx], 0)
            x = x.view(-1, c, h, w).cuda(non_blocking=True)
            if self.ngpu > 1:
            ...
            embeddings_as = embeddings_all[:support_size * nclasses, ...]
            ...
            Q_logits, Q_labels = MultiFactorClustering_logit(embeddings_as, embeddings_q, support_labels, nclasses,
                                                             n_loops=10)
            ###
            loss = 0
            if self.exp_dict["classification_weight"] > 0:
                loss += F.cross_entropy(self.model.classifier(embeddings.view(a, b)), labels.view(-1)) * self.exp_dict[
                    "classification_weight"]

            query_labels = torch.arange(nclasses, device=Q_logits.device).view(1, nclasses).repeat(query_size, 1).view(
                -1)
            loss += self.exp_dict["mfc_weight"] * F.cross_entropy(Q_logits, query_labels) * self.exp_dict[
                "few_shot_weight"]
            loss += (1 - self.exp_dict["mfc_weight"]) * F.cross_entropy(logits, query_labels) * self.exp_dict[
                "few_shot_weight"]

        return loss, v_measure0, v_measure1
    ...
    def train_on_loader(self, data_loader, max_iter=None, debug_plot_path=None):
        ...
        v_measure0_meter = BasicMeter.get("v_measure0").reset()
        v_measure1_meter = BasicMeter.get("v_measure1").reset()
        # Iterate through tasks, each iteration loads n tasks, with n = number of GPU
        self.optimizer.zero_grad()
        for batch_idx, batch in enumerate(data_loader):
            with torch.autograd.set_detect_anomaly(True):
                loss, v_measure0, v_measure1 = self.train_on_batch(batch)
                loss = loss / self.exp_dict["tasks_per_batch"]
                train_loss_meter.update(float(loss), 1)
                v_measure0_meter.update(float(v_measure0),1)
                v_measure1_meter.update(float(v_measure1), 1)
                loss.backward()
                ...
                if batch_idx + 1 == max_iter:
                    break
        return {"train_loss": train_loss_meter.mean(), "v_measure0":v_measure0_meter.mean(), "v_measure_resort":v_measure1_meter.mean()}
    ...
    def generate_label_on_loader(self, data_un_loader, max_iter=None, debug_plot_path=None):
        ...
        features, labels, labels_weight, gts = [], [], [], []
        for i in range(n_inters):
            # self.optimizer.zero_grad()
            batch_un = next(dl_u)
        ...
        return features, labels, labels_weight, {"used_data_ratio": ratio_meter.mean()/bs, "accuracy_used_data": acc_used_meter.mean(),
                                  "accuracy_all_data": acc_all_meter.mean()/bs}, gts
