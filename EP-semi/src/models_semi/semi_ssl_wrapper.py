...
class SSLWrapper(BaseWrapper):
    ...
    def __init__(self, model, n_classes, exp_dict, n_word_emb=300, pretrained_savedir=None, savedir_base=None):
        """ Constructor
        Args:
        ...
        a_type = self.exp_dict["adjust_type"]

        self.model.add_adjust(self.model.output_size, 1, name='self_attention', type=a_type, dropout=dropout)
        self.model.add_adjust(n_word_emb, self.model.output_size, name='adjust_word', type=a_type, dropout=dropout)
        ...
        if pretrained_savedir is None:
            ...
            for exp_hash in os.listdir(savedir_base):
                ...
                if ((loaded_exp_dict["model"]["name"] == 'semi_finetuning' or loaded_exp_dict["model"]["name"] == 'noise_finetuning') and
                    ...
                    # loaded_exp_dict['adjust_type'] == exp_dict["adjust_type"] and
                    loaded_exp_dict['num_per_class'] == exp_dict['num_per_class'] and
                    # loaded_exp_dict['support_size_train'] == support_size_needed and
                    loaded_exp_dict["embedding_prop"] == exp_dict["embedding_prop"]):

                    if "n_distract" in loaded_exp_dict.keys() and "n_distract" in exp_dict.keys():
                        if loaded_exp_dict["n_distract"] != exp_dict["n_distract"]:
                            continue
    ...
    def get_episode_dict(self, batch):
        ...
        if self.exp_dict["trans_encoder"]:
            ...
            features = self.model.trans_encoder(torch.unsqueeze(features, dim=1)).squeeze()
        ...
        if self.exp_dict["word_emb_weight"] > 0:
            ...
            features[:support_size * nclasses] = emb * lambda_c + features[:support_size * nclasses] * (
                    1 - lambda_c)
