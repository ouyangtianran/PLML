...
class EpisodicDataset(Dataset):
    def __init__(self, labels, sampler, size, transforms, unsupervised_loss='rotation', un_transforms=None, un_labels=None, aug=False):
    ...
    def sample_task_list(self):
        ...
        # while (few_shot_task is not None and unlabeled_size==0) or (unlabeled_size > 0 ):
    ...
    def __getitem__(self, idx):
        ...
        if self.labels_gt is not None:
            targets_gt = np.zeros([nclasses * k], dtype=int)
            targets_gt[ordered_argindices] = self.labels_gt[ordered_indices, ...].ravel()
        ...
        sample = {"dataset": self.name,
                  ...
                  "targets_gt": torch.from_numpy(targets_gt) if self.labels_gt is not None else None,
