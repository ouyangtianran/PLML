...
from src.datasets_semi.utils import load_embs, load_split_data,  load_semi_data, load_semi_data_v2
#get_simsiam_trans,
...
class EpisodicMiniImagenetPkl(EpisodicDataset):
    ...
    def __init__(self, data_root, split, sampler, size, transforms, unsupervised_loss='rotation', aug=False):
        """ Constructor
        ...
        super().__init__(labels, sampler, size, transforms, unsupervised_loss, un_transforms, aug=False)
...
class SemiEpisodicMiniImagenetPkl(EpisodicDataset):
    ...
    def __init__(self, data_root, split, sampler, size, transforms, unsupervised_loss='rotation', data_group='train_label', aug=False):
        """ Constructor
        ...
        self.features, self.labels = load_semi_data_v2(self.data_root, features, labels,
                                                       split_semi, data_group)

        self.un_features, self.un_labels = load_semi_data_v2(self.data_root, features, labels,
                                                       split_semi, 'train_unlabel')
        labels = self.labels
        un_transforms = None#get_simsiam_trans(84)

        super().__init__(labels, sampler, size, transforms, unsupervised_loss, un_transforms, self.un_labels, aug=aug)
